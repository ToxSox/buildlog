...
check('Normale Nutzbarkeit entfaellt in E und X Unlimited',
  pts('normalUse').E === 0 && pts('normalUse').XUNL === 0)
check('System documentation erst ab M', pts('sysDoc').E === 0 && pts('sysDoc').S === 0 && pts('sysDoc').M === 10)
...
check('jedes Kriterium hat assess-Modus', CRITERIA.every((c) => ['auto', 'self'].includes(c.assess)))
check('jedes Kriterium ist zweisprachig', CRITERIA.every((c) => c.label.de && c.label.en && c.help.de && c.help.en))
